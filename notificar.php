...
';
    ...
    $notificacoes = $wpdb->get_results($wpdb->prepare("SELECT * FROM $tabela WHERE user_id = %d", $user_id));
...
function meu_plugin_enviar_notificacao($notificacao_id) {
    ...
    // Pegar a notificação do banco de dados
    $notificacao = $wpdb->get_row($wpdb->prepare("SELECT * FROM $tabela WHERE id = %d", $notificacao_id));
...
function meu_plugin_adicionar_coluna_url_redirecionamento() {
    global $wpdb;
    $tabela = $wpdb->prefix . 'meu_plugin_notificacoes';

    // Verificar se a coluna 'url_redirecionamento' já existe
    $coluna_existe = $wpdb->get_results("SHOW COLUMNS FROM $tabela LIKE 'url_redirecionamento'");

    // Se a coluna 'url_redirecionamento' não existe, adicionar
    if (empty($coluna_existe)) {
        $wpdb->query("ALTER TABLE $tabela ADD url_redirecionamento text");
    }
}
add_action('init', 'meu_plugin_adicionar_coluna_url_redirecionamento');

register_activation_hook(__FILE__, 'meu_plugin_adicionar_url_redirecionamento');
